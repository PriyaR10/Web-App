...
$stmt = $connect->prepare("SELECT DISTINCT race_date, stroke_type FROM gala_tournament ORDER BY race_date ASC");
...
<form method="POST">
  <h3>CHOOSE RACE DATE AND STROKE TYPE FROM THE LIST TO SEE THE RESULTS</h3>
  ...
  <select id="race_date" name="race_date">
    ...
    while ($row = $result->fetch_assoc()) {
      $selected = ($row['race_date'] == $_POST['race_date']) ? 'selected' : '';
      echo "<option value='" . $row['race_date'] . "' $selected>" . $row['race_date'] . "</option>";
    }
    ?>
  </select>
  ...
  <select id="stroke_type" name="stroke_type">
    ...
    while ($row = $result->fetch_assoc()) {
      $selected = ($row['stroke_type'] == $_POST['stroke_type']) ? 'selected' : '';
      echo "<option value='" . $row['stroke_type'] . "' $selected>" . $row['stroke_type'] . "</option>";
    }
    ?>
  </select>
...
if (isset($_POST['submit'])) {
  ...
  $stmt = $connect->prepare("SELECT username, duration FROM gala_tournament WHERE race_date = ? AND stroke_type = ? ORDER BY duration ASC");
...
while ($row = $result->fetch_assoc()) {
  ...
  $stmt = $connect->prepare("UPDATE gala_tournament SET position = ? WHERE race_date = ? AND stroke_type = ? AND username = ?");
  ...
  $stmt = $connect->prepare("SELECT * FROM gala_tournament WHERE race_date = ? AND stroke_type = ? ORDER BY position");
