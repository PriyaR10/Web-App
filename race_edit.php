...
$stmt = $connect->prepare("SELECT * FROM race_performance WHERE id = ?");
...
if ($result->num_rows == 1) {
  ...
  if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    // if ($is_coach && !is_null($training_performance)) {
       $sql = "UPDATE race_performance SET username = ?, race_date = ?, stroke_type = ? , distance = ?, duration = ? WHERE id = ?";
       // Prepare the statement
       $stmt = $connect->prepare($sql);
       // Bind the parameters to the statement
      $stmt->bind_param("ssssss", $username, $race_date, $stroke_type , $distance, $duration  , $id);
